<?php
    ...
    $previous_work = mysqli_real_escape_string($conn, $_POST['previous_work']);
    $address = mysqli_real_escape_string($conn, $_POST['address']);
 
    if(!empty($fname) && !empty($lname) && !empty($mob_no) && !empty($email) && !empty($password) && !empty($company_name) && !empty($experience) && !empty($services_provided) && !empty($previous_work) && !empty($address))
    {
        if(filter_var($email,FILTER_VALIDATE_EMAIL))
        {
            //if email is vallid
            //let's check that email already exist in the database
            $sql = mysqli_query($conn, "SELECT email FROM custom_broker WHERE email = '{$email}'");
                ...
                if(isset($_FILES['image']))
                {
                    ...
                    if(in_array($img_ext,$extensions) === true)
                    {
                        ...
                        if( move_uploaded_file($tmp_name,"images/".$new_img_name))
                        {
                            ...
                            $sql2 = mysqli_query($conn, "INSERT INTO custom_broker (unique_id, fname, lname, mob_no, email, password, company_name, experience, services_provided, previous_work, address, img, status)
                                                VALUES ({$random_id}, '{$fname}', '{$lname}', '{$mob_no}', '{$email}', '{$password}', '{$company_name}', '{$experience}', '{$services_provided}', '{$previous_work}', '{$address}', '{$new_img_name}', '{$status}')");
                            if($sql2)
                            {
                                $sql3 = mysqli_query($conn, "SELECT * FROM custom_broker WHERE email = '{$email}'");
